...
module.exports = class Book extends ThrowError{
  constructor(mainWindow) {
    super();


    // 删除项目
    ipcMain.on('del_project', async (sys, query) => {
      const { projectPath } = query;
      fileSystem.rmdirSync(projectPath)
      mainWindow.webContents.send('del_project', {
        code: 200,
        data: '',
      })
    })
    ...
    ipcMain.on('get_books', async (sys, query) => {
      ...
      if (!docPath) {
        this.$error('get_books', 400, '文件夹路径docPath不能为空', mainWindow)
      }
      let sum_data=''
      try {
        sum_data = fileSystem.readFileSync(path.join(docPath + '/LANGS.md'));
      } catch (err) {
        if (!sum_data) {
          this.$error('get_books', 400, '该文件夹下，LANGS.md文件不存在', mainWindow)
        }
      }
    ...
    ipcMain.on('get_directory', async (sys, query) => {
      ...
      if (!bookPath) {
        this.$error('get_directory', 400, 'bookPath为空', mainWindow)
      }
